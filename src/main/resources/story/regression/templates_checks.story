...
When I COMPARE_AGAINST baseline with name `TemplatesDDL` ignoring:
|ELEMENT                                               |
...
When I COMPARE_AGAINST baseline with name `AllTemplates` ignoring:
|ELEMENT                                                                          |
...
When I COMPARE_AGAINST baseline with name `<TemplateCategory>` ignoring:
|ELEMENT                                                                          |
|By.xpath(//*[contains(@title,"Views")]), By.xpath(//*[contains(@title,"Copies")])|
using screenshot configuration:
|scrollableElement             |scrollTimeout|webHeaderToCut|
...
When I change context to element located by `By.xpath(//*[@data-testid="templates-container"])`
When I COMPARE_AGAINST baseline with name `<InfoTemplatePage>` ignoring:
|ELEMENT                                                                          |
|By.xpath(//*[contains(@title,"Views")]), By.xpath(//*[contains(@title,"Copies")])|
using screenshot configuration:
|scrollableElement             |scrollTimeout|webHeaderToCut|
...
When I COMPARE_AGAINST baseline with name `BoardFromTemplate`
When I reset context
...
When I COMPARE_AGAINST baseline with name `CardPopup`
When I reset context
